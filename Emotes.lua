https://discord.gg/U9WbRe7HzX--[[ 
    Source script taken from: https://github.com/Roblox/creator-docs/blob/main/content/en-us/characters/emotes.md

    scriptblox: https://scriptblox.com/script/Universal-Script-SP-I-Emote-Script-48024
]]
...
local State = {
    ...
    targetImages = {},
    AnimationCachePath = "SP/AnimationCache.json",
    AnimationCache = {},
    AnimationListCachePath = "SP/AnimationListCache.json",
    EmoteListCachePath = "SP/EmoteListCache.json",
    CustomAnimationPath = "SP/CustomAnimations.json",
    CustomAnimations = {},
    ...
    EmotePagePath = "SP/EmotePages.json",
    EmotePages = {},
    currentEmotePageName = "Default",
    EmoteDataCachePath = "SP/EmoteDataCache.json"
}
...
function saveAnimationCache()
    if writefile then
        pcall(function()
            if not isfolder("SP") then makefolder("SP") end
...
function SafeLoad(url, name)
    ...
    if not func then
        warn("SP | SafeLoad: Failed to parse " .. (name or "script") .. ": " .. tostring(err))
        return function() end
    end

    local ok, res = pcall(func)
    if not ok then
        warn("SP | SafeLoad: Error executing " .. (name or "script") .. ": " .. tostring(res))
        return function() end
    end
    return res
end

SafeLoad("https://raw.githubusercontent.com/SP/Menu-SP/refs/heads/Script/GUIS/Off-site/Notify.lua", "Notify System")
...
function GetAsset(asset)
    ...
    if assetStr:find("http") then
        ...
        if isfile(path) then
            ...
            if not isfolder("SP/Assets") then 
                pcall(function()
                    if not isfolder("SP") then makefolder("SP") end
                    makefolder("SP/Assets") 
                end)
            end
            ...
            if success and content and content ~= "" then
                ...
                if low:find("<!doctype") or low:find("<html") or low:find("<head") then
                    warn("SP | GetAsset: Downloaded content appears to be HTML. Link might be incorrect: " .. targetUrl)
                    return ""
                end
...
local ConfigPath = "SP/EmoteSettings.json"
...
DiscordBtn.MouseButton1Click:Connect(function()
    setclipboard("https://discord.gg/U9WbRe7HzX")
...
local ThemeConfigPath = "SP/EmoteThemes.json"
...
function ApplyTheme(themeData)
    ...
    if not themeData then
        warn("SP | ApplyTheme: themeData is nil. Falling back to Default.")
    ...
    if not ok then
        warn("SP | ApplyTheme error: " .. tostring(err))
    end
end
...
State.enterCustomAnimationEditor = function(category, animName)
    ...
    if State.currentCustomAnimationName == "Default" then
        getgenv().Notify({ Title = "SP | Error", Content = "Cannot edit Default Animation set. Create a new one!", Duration = 3 })
        return
    end
    ...
    getgenv().Notify({ Title = "SP | Animation Editor", Content = "🖱️ Select an animation from the wheel to set for " .. animName, Duration = 5 })
end
...
SettingsLib.AddIconButton(CustomAnimMgtContainer, "78317476576895", function()
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" then
            ...
            if refreshCustomAnimationState then refreshCustomAnimationState(false) end
            popup:Destroy()
            getgenv().Notify({ Title = "SP | Animation", Content = "✅ Imported custom animations", Duration = 3 })
        else
            getgenv().Notify({ Title = "Error", Content = "Invalid JSON", Duration = 3 })
        end
    end)
...
SettingsLib.AddIconButton(EmotePageMgtContainer, "78317476576895", function() 
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" and d.Type == "EmotePageSet" and type(d.Data) == "table" then
            ...
            popup:Destroy()
            getgenv().Notify({ Title = "SP | Page", Content = "✅ Imported Emote page", Duration = 3 })
        else
        ...
        if s and type(d) == "table" and d.Type then
            ...
            if d.Themes and (typeStr == "All" or typeStr == "Themes") then
                ...
                if themeToApply then
                    ...
                    warn("SP | Missing Default theme during import fallback")
                end
            end
...
pcall(function()
    SafeLoad("https://raw.githubusercontent.com/SP/Hub/Branch/GUIS/count-emote", "Count Emote")
end)
...
function disconnectAllConnections()
    ...
    if ContextActionService then
        ContextActionService:UnbindAction("SP_EmoteWheelHotkeys")
    end
end
    ...
    if index == 1 and randomActive then
        ...
        if State.customAnimationEditorActive then
            ...
            if State.CustomAnimations.Sets[State.currentCustomAnimationName] and cat and name then
                ...
                State.SaveCustomAnimations(State.CustomAnimations)
                getgenv().Notify({ Title = "SP | Saved", Content = "✅ Saved " .. name, Duration = 3 })
                if State.RefreshCustomAnimUI then State.RefreshCustomAnimUI() end
                if refreshCustomAnimationState then refreshCustomAnimationState(true) end
                State.exitCustomAnimationEditor()
            end
...
function fetchAllEmotes()
    ...
    local function fetchFromUrl()
        local success, result = pcall(function()
            local jsonContent = game:HttpGet("https://raw.githubusercontent.com/bpxzshop-BCOX/SP-HUB/refs/heads/main/EmoteSniper.json")
    ...
    task.spawn(function()
        while true do
            ...
            if emoteData then
                ...
                getgenv().Notify({Title = 'SP | Emote', Content = "📦 Emotes loaded", Duration = 3})
                return
            end
            task.wait(3)
        end
...
function fetchAllAnimations()
    ...
    task.spawn(function()
        local success, result = pcall(function()
            local jsonContent = game:HttpGet("https://raw.githubusercontent.com/SP/sniper-Emote/refs/heads/test/AnimationSniper.json")
            if jsonContent and jsonContent ~= "" then
                local data = HttpService:JSONDecode(jsonContent)
                return data.data or {}
            end
        ...
        if offsaleAnimationJson then
            offsaleSuccess, offsaleResult = pcall(function()
                local jsonContent = game:HttpGet("https://raw.githubusercontent.com/SP/sniper-Emote/refs/heads/test/AnimationSniperoffsale.json")
                if jsonContent and jsonContent ~= "" then
                    local data = HttpService:JSONDecode(jsonContent)
                    return data.data or {}
                end
...
function onCharacterAdded(character)
    ...
    if getgenv().autoReloadEnabled and getgenv().lastPlayedAnimation then
        task.spawn(function()
            ...
            getgenv().Notify({
                Title = 'SP | Auto Reload Animation',
...
enterHUDEditor = function()
    ...
    local selectionGui = game:GetService("CoreGui"):FindFirstChild("SP_HUDSelection")
    if not selectionGui then
        selectionGui = Instance.new("ScreenGui")
        selectionGui.Name = "SP_HUDSelection"
    ...
    table.insert(HUD.Connections, exportBtn.MouseButton1Click:Connect(function()
        ...
        setclipboard(HttpService:JSONEncode(data))
        getgenv().Notify({ Title = "SP | HUD Editor", Content = "✅ HUD settings copied", Duration = 2 })
    end))
    ...
    table.insert(HUD.Connections, importBtn.MouseButton1Click:Connect(function()
        ...
        imp.MouseButton1Click:Connect(function()
            ...
            if s and type(d) == "table" then
                ...
                popup:Destroy()
                getgenv().Notify({ Title = "SP | HUD Editor", Content = "✅ HUD settings imported", Duration = 2 })
            else
                getgenv().Notify({ Title = "Error", Content = "Invalid HUD JSON", Duration = 3 })
            end
        end)
    ...
    table.insert(HUD.Connections, resetBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "SP | HUD Editor", Content = "🔄 All designs and frames have been fully reset", Duration = 3 })
    end))
    ...
    table.insert(HUD.Connections, deleteBtn.MouseButton1Click:Connect(function()
        ...
        ApplyUIVisibility()
        pcall(function() updateGUIColors() end)
        getgenv().Notify({ Title = "SP | HUD Editor", Content = "🗑️ Custom Frame deleted", Duration = 2 })
    end))
    ...
    table.insert(HUD.Connections, addBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "SP | HUD Editor", Content = "➕ Custom Frame added!", Duration = 2 })
    end))

    getgenv().Notify({ Title = "SP | HUD Editor", Content = "✏️ Drag elements to reposition", Duration = 5 })
end
...
if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
    SafeLoad("https://raw.githubusercontent.com/SP/Hub/refs/heads/Branch/GUIS/OpenEmote.lua", "Open Emote")
    getgenv().Notify({
        Title = 'SP | Emote Mobile',
